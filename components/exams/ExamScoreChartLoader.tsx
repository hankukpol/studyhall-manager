...
const ExamScoreChart = dynamic(
  () => import("@/components/exams/ExamScoreChart").then((mod) => mod.ExamScoreChart),
  {
    ssr: false,
    loading: () => (
      <div className="mt-5 rounded-[12px] border border-[var(--border)] bg-white p-5">
        <div className="h-48 animate-pulse rounded-[12px] bg-[#F4F4F2]" />
      </div>
    ),
  },
);
    ...
    <section className={portalSectionClass}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          className="rounded-[12px] border border-[var(--border)] bg-white px-4 py-2 text-sm font-medium text-[var(--foreground)] transition hover:bg-[#F4F4F2]"
        >
      ...
      {isChartVisible ? (
        ...
        <div className="mt-5 rounded-[12px] border border-dashed border-[var(--border)] bg-[#F4F4F2] px-5 py-8 text-[13px] text-[var(--muted)]">
          차트는 필요할 때만 로드됩니다.
        </div>
      )}
    </section>
