  ...
  ['components', 'app'].forEach(dir => {
    walkDir(path.join(__dirname, dir), function(filePath) {
      ...
      content = content.replace(/className="([^"]*)"/g, (match, classStr) => {
        ...
        // Target sub-titles that are exactly text-base font-bold text-slate-950
        if (/\btext-base\b/.test(classStr) && /\bfont-bold\b/.test(classStr) && /\btext-slate-950\b/.test(classStr)) {
          let newClassStr = classStr.replace(/\btext-base\b/, 'text-lg');
      ...
      if (content !== original) {
        ...
        console.log(`Updated sub-titles to 18pt in: ${filePath}`);
      }
    });
  });
}
